...
const title = "Customer Login — SentinelChain AI";
const description = "Sign in or register as a customer on SentinelChain AI platform.";
...
function CustomerAuthPage() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => {
      if (data.session) navigate({ to: "/dashboard/customer", replace: true });
    });
  }, [navigate]);
        ...
        <Card className="border-border/70">
          <CardHeader className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="font-display text-xl">Customer Portal</CardTitle>
            </div>
            <CardDescription>
              Sign in or create your customer account to track shipments and manage orders.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-5">
            ...
            <div className="text-center text-sm text-muted-foreground">
              Are you a driver?{" "}
              <Link to="/auth/driver" className="text-primary hover:underline">
                Sign in here
              </Link>
            </div>
          </CardContent>
